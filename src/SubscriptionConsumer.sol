 ...
 */
error SubscriptionConsumer__RequestNotFound(uint256 requestId);
...
contract SubscriptionConsumer is VRFConsumerBaseV2Plus {
        ...
        requestExists(_requestId)
    {
        ...
        IRaffle(raffleAddress).raffleEnds(_randomWords);
    }
    ...
    modifier requestExists(uint256 _requestId) {
        if (!s_requests[_requestId].exists) {
            revert SubscriptionConsumer__RequestNotFound(_requestId);
        }
        _;
    }
}
